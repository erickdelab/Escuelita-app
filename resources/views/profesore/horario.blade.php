...
@section('template_title')
    Horario Semestral - {{ $profesor->nombre }}
@endsection
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h3 class="text-primary fw-bold mb-0">
                <i class="fas fa-calendar-alt me-2"></i> Horario Semestral Docente
            </h3>
            <p class="text-muted mb-0">
                Profesor: <strong>{{ $profesor->n_trabajador }}</strong> - {{ $profesor->nombre }} {{ $profesor->ap_paterno }} {{ $profesor->ap_materno }}
            </p>
        </div>
        {{-- Botón Regresar --}}
        <a href="{{ route('profesores.show', $profesor->n_trabajador) }}" class="btn btn-outline-secondary">
                            ...
                            <tr>
                                ...
                                @for ($dia = 1; $dia <= 5; $dia++)
                                    @if (isset($calendario[$dia][$hora]))
                                            ...
                                            <td rowspan="{{ $clase['duracion'] }}" 
                                                ...
                                                <div class="d-flex flex-column justify-content-center h-100">
                                                    ...
                                                    <div class="small text-muted mb-1" style="font-size: 0.75rem;">
                                                        <i class="fas fa-layer-group"></i> Grupo: {{ $clase['grupo_id'] }} (Sem {{ $clase['semestre'] }})
                                                    </div>
                                        ...
                                        @else
                                            {{-- Esta hora está ocupada por un rowspan anterior --}}
                                        @endif
